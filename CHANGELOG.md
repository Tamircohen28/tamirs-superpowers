...
open the whole time — while master's own release process independently, and
concurrently, cut real `3.6.3`, `3.7.0` and `3.8.0` releases from three separate
sibling PRs (#153/#152, #150, and #149 respectively — see the three entries below this
one). Reusing 3.6.3, 3.6.4, 3.6.5, 3.7.0 or 3.8.0 for this content would collide with an
already-shipped version, so this reconciliation lands as a fresh `3.8.1`, the next patch
after master's newest real cut at merge time. The per-release narrative below is the
merged, de-duplicated story; nothing here changes `hooks/`, `skills/`, or `agents/`
content, so the co-authored notes on individual host fixes are compressed.
