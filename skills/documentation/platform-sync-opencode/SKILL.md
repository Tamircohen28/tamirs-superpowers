...
description: >-
  DEPRECATED compatibility shim. OpenCode analysis now runs inside the single
...
when_to_use: |
  ...
  It exists only so an existing caller that names `platform-sync-opencode` still resolves.
...
# platform-sync-opencode (deprecated shim)

This skill no longer contains any OpenCode analysis logic.
...
are now **data**, in
`skills/documentation/platform-sync/references/platforms/opencode.md`, and the single loop
...
1. Invoke `tamirs-superpowers:platform-sync` via the Skill tool, scoped to `opencode`.
2. Return its `OpenCode` section unchanged.
...
| Was | Now |
|---|---|
| `platform-sync-opencode` SKILL.md | `platform-sync/references/platforms/opencode.md` (data) |
