...
#
# WHAT IS SCANNED
#   Every *.yml, *.yaml, *.yml.tmpl, *.yaml.tmpl and *.md in the tree (.git,
#   node_modules and .venv pruned). The `.tmpl` variants are narrowed to
#   workflow-shaped names, not a bare `*.tmpl`, because this plugin also ships
#   `check-*.sh.tmpl` -- shell-script templates copied verbatim into a
#   scaffolded repo's scripts/, one of them this very checker. A bare `*.tmpl`
#   glob read this script's own self-test fixtures (literal `uses: ...@v7`
#   lines it writes to prove the detector fires) as real workflow content the
#   moment the checker gained a template copy of itself, and reported its own
#   test data as a violation. `.md` is scanned because template documentation
#   carries real workflow bodies in fenced blocks -- and ONLY fenced blocks
#   there, since prose about a mutable ref is not a mutable ref.
#
    ...
    while IFS= read -r line; do
      ...
      case "${ref##*@}" in
        ...
        *) printf '%s:%s:%s\n' "$f" "$n" "$ref" ;;
           ...
           \( -name '*.yml' -o -name '*.yaml' -o -name '*.yml.tmpl' -o -name '*.yaml.tmpl' -o -name '*.md' \) \
...
jobs:
  a:
    steps:
      - uses: tmpl/act@v1
TMPL
  # A shell-script template (this checker's own shape, once it gained a
  # `.tmpl` copy of itself) is not a workflow-shaped template. Its self-test
  # necessarily writes literal `uses: ...@v7` lines to prove the detector
  # fires, and a bare `*.tmpl` glob read those as real refs the moment this
  # script grew a `check-action-pinning.sh.tmpl` sibling.
  cat > "$tmp/bad/check-something.sh.tmpl" <<'SH'
#!/usr/bin/env bash
# self-test fixture: - uses: selftest/not-a-real-ref@v1
SH
  out="$(scan "$tmp/bad")"
  [ "$(printf '%s\n' "$out" | grep -c 'checkout@v7')" = 1 ] \
  ...
  [ "$(printf '%s\n' "$out" | grep -c 'selftest/not-a-real-ref')" = 0 ] \
    || { echo "  self-test: a shell-script .sh.tmpl was scanned as a workflow" >&2; rc=1; }
