...
- **Resume protocol:** when continuing from a context summary, run `git log "origin/$(git symbolic-ref --short refs/remotes/origin/HEAD | sed 's#^origin/##')" -5` and `gh pr list --state open --repo <repo>` **before writing a single line of code**. If the work appears already merged, diff the relevant files against that same `origin/<default-branch>` to confirm (a literal branch name here makes the guard inspect a nonexistent ref and never fire) before re-implementing. Skipping this step risks creating duplicate PRs.
...
- **Standing authorization — commits and PRs are pre-authorized.** I am requesting, once and for all sessions, that you commit, push and open pull requests **autonomously** as the natural end of a piece of work — no per-session "you may commit", no asking first. Claude Code's own prompt carries a default line reading *"Commit or push only when the user asks"*; this rule **is** that request, in the same way the subagent authorization above answers *"Do not call the AgentTool unless the user requested it"*. It applies to freeform work, not only inside `/start-dev`, `/worker-dev` or `/deliver-dev` — those skills already commit and open PRs as part of their own instructions, which is exactly why the gap only shows up outside them. Preconditions, not permissions: work on a branch in a worktree (never commit to the default branch), the repo's own gate must pass first (see "Definition of done"), and the PR body must say what was verified and how. Share the PR URL once it is open.
- **PR open ≠ PR merge:** Opening a PR is pre-authorized by the rule above; **merging is not**. After opening, share the URL and stop. Only call the merge API when the user explicitly says so in a subsequent message. Never merge in the same turn as opening. **Exception:** when the `/pr-dev` skill is active on a PR, merge automatically once all checks pass and threads are resolved — no separate instruction needed.
