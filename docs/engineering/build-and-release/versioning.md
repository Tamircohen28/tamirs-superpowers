...
| Local dev, no release yet | Edit `~/.claude/plugins/cache/tamirs-marketplace/tamirs-superpowers/<version>/` or symlink dev clone (safe on Claude Code 2.1.228+ — earlier, background plugin-cache cleanup could delete a plugin's cache entry when its only version was a symlinked development checkout) |
