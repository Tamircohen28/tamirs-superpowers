...
Verified **2026-09-15** — Claude Code reviewed against the official changelog through **2.1.272** (released 2026-09-15), confirmed as the current npm dist-tag. This cycle's automation environment DID have a live `claude` CLI (installed build reports 2.1.273, newer than this review's 2.1.272 target; changelog data was available only through 2.1.272 this cycle, so the documented baseline stops there rather than guessing at an unreviewed release), so `validated_against` advances to **2.1.272** on real evidence: `claude plugin validate .` passed and `bash scripts/doctor.sh .` reported the install healthy (the same 4 environment-dependent optional gaps as prior cycles — `gh`, `shellcheck`, GitHub automation, notifications). `reviewed_through`/`latest_known` also advance to **2.1.272**. This cycle's new releases are 2.1.271 and 2.1.272, on top of the 2.1.259–2.1.270 review recorded below. 2.1.272 (bug-fixes-and-reliability-only) has no adoptable item. 2.1.271: reviewed and deliberately not adopted — `omitClaudeMd` (new agent frontmatter field) checked against all ten `agents/*.md` and left unset, since each still needs the *target* project's own `CLAUDE.md` for the domain conventions it reviews or implements against; per-command `allowed_domains` for Bash/PowerShell/Monitor in auto mode with sandboxing, checked against `platforms/claude/settings.d/` (no PowerShell rules, no network-scoped Bash rules beyond `gh`/`git`'s own auth, no auto-mode-with-sandboxing guidance here); `--accept-command <sha256>` for `claude plugin install/update`, checked against `scripts/update.sh`/`scripts/uninstall.sh`'s existing `--json` calls, neither of which passes `-y` or hits an interactive confirmation today. **Adopted (documentation, each matching a mechanism this repo already relies on):** the Monitor watch deadline change (always ≤30 minutes, replacing the no-timeout `persistent` option) — `skills/dev-workflow/pr-dev/references/ci-monitor-loop.md`'s one Monitor wrapper already sets `timeout_ms: 1800000` and never set `persistent`; a background command no longer duplicate-starting after the conversation compacts, relevant to a long `orchestrate-dev`/`worker-dev` fan-out; a stale `.git/config.lock` after a sandboxed command failure no longer breaking `git checkout -b`/`git push -u`/`git config` (Linux), relevant since `permissions-allow.json` pre-allows exactly those commands to avoid this class of friction; and cross-session `SendMessage` delivery giving the headless sender a notice instead of holding a message silently, documented in `docs/user/cross-platform-workflow.md` alongside the existing 2.1.224–2.1.247 `SendMessage` narrative. No breaking changes or deprecations in the delta. Full narrative including the 2.1.259–2.1.270 items and what was reviewed and found not applicable then: `CLAUDE.md`'s Subagents, Hooks, MCP, Marketplace cache, Skill frontmatter and Remote and headless Claude sessions sections, `docs/user/cross-platform-workflow.md`, `docs/engineering/build-and-release/development-workflow.md`'s Running checks section, and `platform-targets.json`'s `verification_method`. Cursor was verified against **3.18.9** on 2026-08-30; Gemini CLI remains directly validated at **0.55.1** and OpenCode at **1.18.11** (maintainer-machine run, 2026-08-03). Codex releases are reviewed through **0.152.1** (released 2026-09-01), while direct CLI validation remains at **0.146.0** until the next maintainer-machine run. The 0.150–0.152 delta does not require a plugin-manifest migration: native task references and `Interrupt` hooks are host-specific opportunities; optional-MCP discovery grace, MCP-result interception, per-repository plugin catalog configuration, package-style MCP server names, per-tool MCP `output_token_limit`, and longer app-server shell-command timeouts are native Codex capabilities. `tools.update_plan.enabled` is opt-in starting in 0.152; this repo does not depend on that tool, so no config migration is required. The shared cross-target hook bundle remains unchanged so Codex-only lifecycle semantics cannot alter Claude/Cursor behavior. One correction worth recording: the Codex manifest `hooks` field is not a 0.147.0 feature. 0.147.0 is portable agent plugins (catalog install); plugin-manifest hooks were wired into the hook runtime by openai/codex PR #19705, merged 2026-04-28, about twenty releases earlier. `core/capabilities/platforms.json` recorded codex hooks as `since: 0.147.0` on that confusion, ahead of the 0.146.0 actually validated. The exact floor has never been established here, so the registry now claims none rather than a wrong one.

| Surface | Min supported | Validated against | Reviewed through | Latest known | Install guide |
|----------|---------------|-------------------|------------------|--------------|---------------|
| Claude Code | 2.0.0 | 2.1.272 | 2.1.272 | 2.1.272 | [claude-code.md](../../user/install/claude-code.md) |
