...
Verified **2026-09-12** — Claude Code reviewed against the official changelog through **2.1.270** (released 2026-09-12), confirmed as the current npm dist-tag. This cycle's automation environment DID have a live `claude` CLI, so `validated_against` advances to **2.1.270** on real evidence: `claude plugin validate .` passed and `bash scripts/doctor.sh .` reported the install healthy (4 environment-dependent optional gaps only — `gh`, `shellcheck`, GitHub automation, notifications). `reviewed_through`/`latest_known` also advance to **2.1.270**. This cycle's new releases are 2.1.269 and 2.1.270, on top of the 2.1.259–2.1.268 review recorded below. 2.1.269: `claude plugin eval` (score a plugin's own eval suite) is highly relevant given this plugin's 27 skills, but its `case.yaml`/`prompt.md`+`graders` format is a different shape from this repo's existing per-skill `evals/evals.json`/`trigger-evals.json` trigger-accuracy harness, so wiring it is tracked as a Future opportunity rather than adopted this cycle; a `PreToolUse` deny rule and the write-path check behind `Edit()` now also apply to a Bash `tee` write, and a `Bash(tee:*)` allow rule is correctly scoped — this closes, host-side, exactly the gap `hooks/guard-sensitive-files.sh`'s header comment documents and already covers independently via `hooks/lib/write-targets.py`'s own `tee` parsing; neither this repo's permission files declare a `Bash(tee:*)` rule; a deny/ask rule starting with a bare `!` leaking beyond its settings source is fixed (none of this repo's rules start with `!`); the attribution-reminder text no longer overrides a project's CLAUDE.md/memory instruction against a trailer (this repo's own CLAUDE.md asks *for* the trailer, so never in conflict); plugin archive extraction is hardened against other-local-user readability, world-writable bits, and stale re-extraction leftovers (host-side); `/output-style [name]` now works over Remote Control and in cloud/headless sessions (interactive convenience, nothing to adopt); `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS` governs the built-in Workflow tool, which this repo does not use (it ships its own orchestrate-dev/worker-dev fan-out). Reviewed and found not applicable: plugin LSP `shutdown`/`exit` fix (no LSP server shipped); org managed-settings plugin loading in headless sessions (this plugin distributes via marketplace add + install, not managed settings); synced plugin MCP servers on remote-session resume (this repo's one MCP server is its own stdio server, not synced); `/ultrareview --post` (no `ultrareview` skill here); `anthropic-skills:<name>` cloud-sync naming (no doc here names a bare id it would shadow); Artifact-DB scratchpad reads (no shipped skill uses an Artifact database). 2.1.270 (bug fix, a regression in 2.1.269): read-only git commands in Bash no longer unexpectedly ask for permission after a long-running session — relevant since `permissions-allow.json` deliberately pre-allows several git commands to avoid exactly this friction; a compatibility bump only. Full narrative including the 2.1.259–2.1.268 items and what was reviewed and found not applicable then: `CLAUDE.md`'s Subagents, Hooks, MCP, Marketplace cache, Skill frontmatter and Remote and headless Claude sessions sections, `docs/engineering/build-and-release/development-workflow.md`'s Running checks section, and `platform-targets.json`'s `verification_method`. Cursor was verified against **3.18.9** on 2026-08-30; Gemini CLI remains directly validated at **0.55.1** and OpenCode at **1.18.11** (maintainer-machine run, 2026-08-03). Codex releases are reviewed through **0.152.1** (released 2026-09-01), while direct CLI validation remains at **0.146.0** until the next maintainer-machine run. The 0.150–0.152 delta does not require a plugin-manifest migration: native task references and `Interrupt` hooks are host-specific opportunities; optional-MCP discovery grace, MCP-result interception, per-repository plugin catalog configuration, package-style MCP server names, per-tool MCP `output_token_limit`, and longer app-server shell-command timeouts are native Codex capabilities. `tools.update_plan.enabled` is opt-in starting in 0.152; this repo does not depend on that tool, so no config migration is required. The shared cross-target hook bundle remains unchanged so Codex-only lifecycle semantics cannot alter Claude/Cursor behavior. One correction worth recording: the Codex manifest `hooks` field is not a 0.147.0 feature. 0.147.0 is portable agent plugins (catalog install); plugin-manifest hooks were wired into the hook runtime by openai/codex PR #19705, merged 2026-04-28, about twenty releases earlier. `core/capabilities/platforms.json` recorded codex hooks as `since: 0.147.0` on that confusion, ahead of the 0.146.0 actually validated. The exact floor has never been established here, so the registry now claims none rather than a wrong one.

| Surface | Min supported | Validated against | Reviewed through | Latest known | Install guide |
|----------|---------------|-------------------|------------------|--------------|---------------|
| Claude Code | 2.0.0 | 2.1.270 | 2.1.270 | 2.1.270 | [claude-code.md](../../user/install/claude-code.md) |
