...
Verified **2026-09-10** — Claude Code reviewed against the official changelog through **2.1.268** (released 2026-09-10), confirmed as the current npm dist-tag. This cycle's automation environment DID have a live `claude` CLI, so `validated_against` advances to **2.1.268** on real evidence: `claude plugin validate .` passed and `bash scripts/doctor.sh .` reported the install healthy (4 environment-dependent optional gaps only — `gh`, `shellcheck`, GitHub automation, notifications). `reviewed_through`/`latest_known` also advance to **2.1.268**. This cycle's new release is 2.1.268, on top of the 2.1.259–2.1.267 review recorded below: PermissionRequest hooks now fire in `--print` (headless) mode (no PermissionRequest hook is wired here); `CLAUDE_CODE_SESSIONEND_HOOKS_TIMEOUT_MS` now actually extends a SessionEnd hook with no per-hook `timeout` (every SessionEnd hook here already sets one, so unaffected); `/mcp`/`/plugin`/MCP-login errors no longer leak a secret resolved from a `${VAR}` placeholder (this repo's `.mcp.json` uses none); `claude plugin install/uninstall/update/enable/disable` gained `--json` and `claude plugin list --json` rows gained `errorDetails`/`noteDetails` — adopted in `scripts/update.sh`/`scripts/uninstall.sh` for specific failure diagnostics instead of a generic fallback message; `/plugin install/enable/disable` no longer needing `/reload-plugins` after closing the interactive menu (distinct from, and does not change, `hooks/plugin-reload-reminder.sh`'s reminder for local file edits made outside that menu); a respawned in-process teammate no longer inherits tools/system-prompt from a same-named untrusted agent file (this repo's `agents/*.md` are trusted and versioned, so not exercised); a git-source-URL secret-leak fix and a marketplace entry-path containment hardening are host-side security fixes with nothing to change here. Full narrative including the 2.1.259–2.1.267 items and what was reviewed and found not applicable: `CLAUDE.md`'s Subagents, Hooks, MCP, Marketplace cache and Skill frontmatter sections, `docs/engineering/build-and-release/development-workflow.md`'s Running checks section, and `platform-targets.json`'s `verification_method`. Cursor was verified against **3.18.9** on 2026-08-30; Gemini CLI remains directly validated at **0.55.1** and OpenCode at **1.18.11** (maintainer-machine run, 2026-08-03). Codex releases are reviewed through **0.152.1** (released 2026-09-01), while direct CLI validation remains at **0.146.0** until the next maintainer-machine run. The 0.150–0.152 delta does not require a plugin-manifest migration: native task references and `Interrupt` hooks are host-specific opportunities; optional-MCP discovery grace, MCP-result interception, per-repository plugin catalog configuration, package-style MCP server names, per-tool MCP `output_token_limit`, and longer app-server shell-command timeouts are native Codex capabilities. `tools.update_plan.enabled` is opt-in starting in 0.152; this repo does not depend on that tool, so no config migration is required. The shared cross-target hook bundle remains unchanged so Codex-only lifecycle semantics cannot alter Claude/Cursor behavior. One correction worth recording: the Codex manifest `hooks` field is not a 0.147.0 feature. 0.147.0 is portable agent plugins (catalog install); plugin-manifest hooks were wired into the hook runtime by openai/codex PR #19705, merged 2026-04-28, about twenty releases earlier. `core/capabilities/platforms.json` recorded codex hooks as `since: 0.147.0` on that confusion, ahead of the 0.146.0 actually validated. The exact floor has never been established here, so the registry now claims none rather than a wrong one.

| Surface | Min supported | Validated against | Reviewed through | Latest known | Install guide |
|----------|---------------|-------------------|------------------|--------------|---------------|
| Claude Code | 2.0.0 | 2.1.268 | 2.1.268 | 2.1.268 | [claude-code.md](../../user/install/claude-code.md) |
