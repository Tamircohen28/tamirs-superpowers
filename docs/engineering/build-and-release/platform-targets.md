...
Verified **2026-09-17** — Claude Code reviewed against the official changelog through **2.1.274** (released 2026-09-17), confirmed as the current npm dist-tag, with a live `claude` CLI in this cycle's automation environment reporting exactly `2.1.274` and `claude plugin validate .` re-run clean against it. This entry had drifted: master itself stayed pinned at 2.1.263 (verified 2026-09-06/07) while 2.1.267–2.1.272 review work accumulated on this repo's rolling `claude-code-update` PR without landing; a prior cycle (2026-09-16) reconciled that PR against current master and extended coverage to 2.1.273 on changelog evidence only, no live CLI being available then. `validated_against`/`reviewed_through`/`latest_known` now all advance to **2.1.274** together, this time on real evidence. Highlights across the 2.1.263→2.1.274 range: `/skill-doctor` and the worktree-isolation permission fix (2.1.259–2.1.263); the `effort:` frontmatter fix relevant to 27 `SKILL.md` files here, and a marketplace entry-path containment hardening (2.1.267); `claude plugin install/uninstall/update --json`, now adopted in `scripts/update.sh`/`scripts/uninstall.sh` (2.1.268); a `Bash(tee:*)`/`Edit()` write-path fix that closes, host-side, exactly the gap `hooks/guard-sensitive-files.sh` already covers independently, and `claude plugin eval` reviewed but tracked as a Future opportunity rather than adopted blind, since its case/grader shape doesn't match this repo's existing skill-trigger-accuracy harness (2.1.269, the largest delta in the range); a same-week permission-prompt regression fix (2.1.270, bug-fix-only); the Monitor watch deadline, stale `.git/config.lock` fix, and `SendMessage` delivery-notice improvement, all matching mechanisms this repo already relies on, while `omitClaudeMd` and auto-mode `allowed_domains` were reviewed and deliberately not adopted (2.1.271); bug-fixes-only (2.1.272); a revert of a 2.1.268 Bash-`eval`/`env -C` deny-rule check that this repo declares no rule pattern for, reviewed and found not applicable (2.1.273); and, this cycle, a sub-agent progress-summary fix directly relevant to `worker-dev`'s handoff vocabulary, a `claude agents` CLI flag-loss fix (not exercised by any script here), and `CLAUDE_CODE_MCP_STARTUP_WAIT_MS`, now documented in `CLAUDE.md`'s MCP bullet as a knob for `scripts/github-mcp.sh`'s Docker fallback path (2.1.274). No breaking changes or deprecations anywhere in the range. Full narrative: `CLAUDE.md`'s Subagents, Hooks, MCP, Marketplace cache, Skill frontmatter and Remote and headless Claude sessions sections, `docs/user/cross-platform-workflow.md`, and `platform-targets.json`'s `verification_method`. Cursor was verified against **3.20.17** on 2026-09-13 (changelog through Projects **2026-09-10**); Gemini CLI remains directly validated at **0.55.1** and OpenCode at **1.18.11** (maintainer-machine run, 2026-08-03). Codex releases are reviewed through **0.152.1** (released 2026-09-01), while direct CLI validation remains at **0.146.0** until the next maintainer-machine run. The 0.150–0.152 delta does not require a plugin-manifest migration: native task references and `Interrupt` hooks are host-specific opportunities; optional-MCP discovery grace, MCP-result interception, per-repository plugin catalog configuration, package-style MCP server names, per-tool MCP `output_token_limit`, and longer app-server shell-command timeouts are native Codex capabilities. `tools.update_plan.enabled` is opt-in starting in 0.152; this repo does not depend on that tool, so no config migration is required. The shared cross-target hook bundle remains unchanged so Codex-only lifecycle semantics cannot alter Claude/Cursor behavior. One correction worth recording: the Codex manifest `hooks` field is not a 0.147.0 feature. 0.147.0 is portable agent plugins (catalog install); plugin-manifest hooks were wired into the hook runtime by openai/codex PR #19705, merged 2026-04-28, about twenty releases earlier. `core/capabilities/platforms.json` recorded codex hooks as `since: 0.147.0` on that confusion, ahead of the 0.146.0 actually validated. The exact floor has never been established here, so the registry now claims none rather than a wrong one.

| Surface | Min supported | Validated against | Reviewed through | Latest known | Install guide |
|----------|---------------|-------------------|------------------|--------------|---------------|
| Claude Code | 2.0.0 | 2.1.274 | 2.1.274 | 2.1.274 | [claude-code.md](../../user/install/claude-code.md) |
| Cursor | 3.20.17 | 3.20.17 | 3.20.17 | 3.20.17 | [cursor.md](../../user/install/cursor.md) |
