...
The test of this contract is a number: **adding a seventh harness should touch about
five files, none of which is a skill.** If a new platform ever requires editing skills,
the contract has been violated and the violation is the bug.
...
[`core/capabilities/platforms.json`](../../../core/capabilities/platforms.json), and one
entry in `docs/engineering/build-and-release/platform-targets.json`.
...
registry_key: gemini_cli   # the key under `platforms` in the capability registry
...
platform entry in the registry, and `scripts/check-capability-registry.sh` fails without
them. `install.type` is one of `marketplace`, `extension`, `plugin-dir`, `path`,
`symlink`, `manual`.
...
All nineteen capability rows, with honest statuses. Anything unmeasured is `unknown`
with a note — see [capability-model.md](capability-model.md). This is where the real work
is, and it is bounded: it is research about the platform, not code.
...
`docs/user/install/<id>.md`, declared as `install_doc` in `platform-targets.json` (which
`scripts/check-doc-claims.sh` verifies exists) and as `install.doc` in the registry.
...
failure — otherwise every contributor needs all six harnesses installed.
...
| **A new runtime dependency** | Adding Node to the framework because one platform's extension template happens to include one would tax all six platforms for one. | Keep adapter content declarative. |
...
- [ ] Registry entry exists, all 19 rows explicit, no `native` without a `validation` command.
- [ ] `bash scripts/check-capability-registry.sh` passes.
- [ ] `platform-targets.json` lists it in `supported_targets` with a real `install_doc`.
